create or replace procedure INSIDESALES_4(giorno IN NUMBER) is
       campy NUMBER(8);
...
begin
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS0
      ...
      WHERE UPLN_ACCT_KEY = -100 -- reporting to the house
          AND cmp.mrkt_id = 34
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS1
      SELECT i0.*,
             c.ACCOUNT_TYPE,
             c.ACCOUNT_NUMBER,
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS_Deliveries
...
-- BRING DRAFT QV - NOTE THAT THIS BRINGS DRAFT QV FROM BOTH CURRENT AND NEXT CAMPAIGNS
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS_CurrentCampB
...
-- BRING COMMUNICATION PREFERENCES
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS3
      SELECT i2.*,
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS5
...
EXECUTE IMMEDIATE 'INSERT INTO IS6
      SELECT i5.*,
             f.QV AS QV_PRIOR1,
             f2.QV AS QV_PRIOR2,
             f3.QV AS QV_PRIOR3,
...
EXECUTE IMMEDIATE 'INSERT INTO IS7
      SELECT i6.*,
...
-- BRING BROCHURE SALES DATA ------------------------------------
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS_BrochureSales1
      --CREATE TABLE IS_BrochureSales1 AS
      select    a15.SGMTN_TIER_KEY  SGMTN_TIER_KEY,
                     a15.MRKT_ID  MRKT_ID,
                     a18.SGMTN_TIER_DESC_TXT  SGMTN_TIER_DESC_TXT,
                     a18.SGMTN_TIER_CD  SGMTN_TIER_CD,
                     a13.FLD_SLS_CMPGN_PERD_ID  FLD_SLS_CMPGN_PERD_ID,
                     a13.MRKT_ID  MRKT_ID0,
                     a16.CMPGN_PERD_DESC_TXT  FLD_SLS_CMPGN_PRED_DESC_TXT,
                     a11.MRKT_FSC_KEY  MRKT_FSC_KEY,
                     a17.FSC_DESC_TXT  FSC_DESC_TXT,
                     a17.FSC_CD  FSC_CD,
                     a17.MRKT_ID  MRKT_ID1,
                     a12.ACCT_NR  ACCT_NR,
                     sum(a11.NTA_AMT)  NTA_AMT,
                     sum(a11.UNIT_QTY)  Unit_Quantity
      from      CDW_DSS.V_FACT_INVC_LINE@cdw.prod.sap.lnk         a11
                     join        CDW_DSS.V_DIM_REP@cdw.prod.sap.lnk    a12
                       on        (a11.ACCT_KEY = a12.ACCT_KEY and
                     a11.MRKT_ID = a12.MRKT_ID)
                     join        CDW_DSS.V_DIM_MRKT_INV_DT_BRDG_FLDSLS@cdw.prod.sap.lnk a13
                       on        (a11.FLD_SLS_CMPGN_PERD_ID = a13.FLD_SLS_CMPGN_PERD_ID and
                     a11.INVC_DT_KEY = a13.INVC_DT_KEY and
                     a11.MRKT_ID = a13.MRKT_ID)
                     join        CDW_DSS.V_DIM_DLY_BILNG_CNTRL@cdw.prod.sap.lnk          a14
                       on        (a11.BILNG_ACTN_KEY = a14.BILNG_ACTN_KEY and
                     a11.BILNG_TRAN_KEY = a14.BILNG_TRAN_KEY and
                     a11.MRKT_ID = a14.MRKT_ID)
                     join        CDW_DSS.V_FACT_MRKT_ACCT_CMPGN_SUMRY@cdw.prod.sap.lnk    a15
                       on        (a11.ACCT_KEY = a15.ACCT_KEY and
                     a11.CRNCY_TYP_ID = a15.CRNCY_TYP_ID and
                     a11.FLD_SLS_CMPGN_PERD_ID = a15.FLD_SLS_CMPGN_PERD_ID and
                     a11.MRKT_ID = a15.MRKT_ID)
                     join        CDW_DSS.V_DIM_MRKT_CMPGN_FLD_SLS@cdw.prod.sap.lnk a16
                       on        (a13.FLD_SLS_CMPGN_PERD_ID = a16.FLD_SLS_CMPGN_PERD_ID and
                     a13.MRKT_ID = a16.MRKT_ID)
                     join       CDW_DSS.V_DIM_MRKT_FSC@cdw.prod.sap.lnk           a17
                       on        (a11.MRKT_FSC_KEY = a17.MRKT_FSC_KEY and
                     a11.MRKT_ID = a17.MRKT_ID)
                     join        CDW_DSS.V_DIM_SGMTN_TIER@cdw.prod.sap.lnk  a18
                       on        (a15.MRKT_ID = a18.MRKT_ID and
                     a15.SGMTN_TIER_KEY = a18.SGMTN_TIER_KEY)
      where    (a11.MRKT_ID in (34)
      and to_number(substr(a13.FLD_SLS_CMPGN_PERD_ID,1,4) || substr(a13.FLD_SLS_CMPGN_PERD_ID,7,2)) >= ' || campaign_pc3 || '
      and a11.CRNCY_TYP_ID in (1)
      and a14.SLS_GRP_TYP_ID in (3, 2)
      and a11.MRKT_FSC_KEY IN (
                              191482294730,  191482298730,  191482302730,
                              191482306730,  191482326730,  191482330730,
                              191482334730,  191482338730,  191482358730,
                              191482362730,  191482366730,  191482370730
                              ))
      group by       a15.SGMTN_TIER_KEY,
                     a15.MRKT_ID,
                     a18.SGMTN_TIER_DESC_TXT,
                     a18.SGMTN_TIER_CD,
                     a13.FLD_SLS_CMPGN_PERD_ID,
                     a13.MRKT_ID,
                     a16.CMPGN_PERD_DESC_TXT,
                     a11.MRKT_FSC_KEY,
                     a17.FSC_DESC_TXT,
                     a17.FSC_CD,
                     a17.MRKT_ID,
...
/* AFTER EVERY CAMPAIGN FLIPOVER RUN THIS QUERY TO GET THE NEW LIST OF BROCHURE FSCs AND PASTE'EM IN THE QUERY ABOVE
SELECT mrkt_fsc_key
FROM CDW_DSS.V_DIM_MRKT_FSC@cdw.prod.sap.lnk
WHERE fsc_cd IN
      (
      SELECT FSC_CD
      FROM is_brochure_fsc
      WHERE campaign BETWEEN 201806 AND 201806 + 1
      );*/

      EXECUTE IMMEDIATE 'TRUNCATE TABLE IS_BrochureSales2';
      EXECUTE IMMEDIATE 'INSERT INTO IS_BrochureSales2
      SELECT b1.SGMTN_TIER_KEY, b1.MRKT_ID, b1.SGMTN_TIER_DESC_TXT, b1.SGMTN_TIER_CD, b1.FLD_SLS_CMPGN_PERD_ID, b1.MRKT_ID0,
             b1.FLD_SLS_CMPGN_PRED_DESC_TXT, b1.MRKT_FSC_KEY, b1.FSC_DESC_TXT, b1.FSC_CD, b1.MRKT_ID1, b1.ACCT_NR,
             b1.NTA_AMT, b1.UNIT_QUANTITY,
             fsc.campaign AS BROCHURE_CAMPAIGN
      FROM IS_BrochureSales1 b1
      LEFT JOIN IS_BROCHURE_FSC fsc
      ON b1.fsc_cd = fsc.fsc_cd
      GROUP BY b1.SGMTN_TIER_KEY, b1.MRKT_ID, b1.SGMTN_TIER_DESC_TXT, b1.SGMTN_TIER_CD, b1.FLD_SLS_CMPGN_PERD_ID, b1.MRKT_ID0,
             b1.FLD_SLS_CMPGN_PRED_DESC_TXT, b1.MRKT_FSC_KEY, b1.FSC_DESC_TXT, b1.FSC_CD, b1.MRKT_ID1, b1.ACCT_NR,
             b1.NTA_AMT, b1.UNIT_QUANTITY,
             fsc.campaign';
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS_BrochureSales2_FREE
      SELECT ACCT_NR, BROCHURE_CAMPAIGN, SUM(UNIT_QUANTITY) AS BROCHURE_UNIT
      FROM IS_BrochureSales2
      WHERE NTA_AMT = 0
      GROUP BY ACCT_NR, BROCHURE_CAMPAIGN';

      EXECUTE IMMEDIATE 'TRUNCATE TABLE IS_BrochureSales2_PAID';
      EXECUTE IMMEDIATE 'INSERT INTO IS_BrochureSales2_PAID
      SELECT ACCT_NR, BROCHURE_CAMPAIGN, SUM(UNIT_QUANTITY) AS BROCHURE_UNIT
      FROM IS_BrochureSales2
      WHERE NTA_AMT > 0
      GROUP BY ACCT_NR, BROCHURE_CAMPAIGN';
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS8
      SELECT   i7.*,
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS9
      ...
      EXECUTE IMMEDIATE 'INSERT INTO IS10
...
/*      EXECUTE IMMEDIATE 'INSERT INTO IS11
      SELECT i10.*, b.BROCHURE_UNIT AS FREE_BROCHURE_NEXT_CAMP,
      ...
      WHERE CAMPAIGN <= campaign_pc2 -- REFIX!!!!!!!!!!!!!! campaign_pc2
      ...
      WHERE (
            ...
            OR
            (STATUS = 'Deactivated'
            AND INACTIVE_CAMPAIGNS BETWEEN 7 AND 10)
            )
            AND QV = 0
            AND POTENTIAL_QV = 0
      ...
      EXECUTE IMMEDIATE 'ALTER TABLE IS_CAMPAIGN
      ADD CONSTRAINT acc PRIMARY KEY(ACCOUNT_KEY, DAY_OF_CAMPAIGN, CAMPAIGN)';
